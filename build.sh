...
show_help() {
    ...
    echo "    patch_render      Render a .patch to a WAV (gate closed: the graph, not a note) [tool]"
    echo "    patch_play        Play notes through a .patch and render a WAV [tool]"
    echo "    patch_fx          Run a WAV through an effect .patch [tool]"
    echo "    patch_live        Play a .patch live, reloading it whenever the file changes [tool]"
    echo "    patch_midi        Render a MIDI file through a .patch [tool]"
...
ALL_TARGETS=(
    ...
    "patch_render|patch_render.lpr|$TOOLS_DIR|patch_render|tool"
    "patch_play|patch_play.lpr|$TOOLS_DIR|patch_play|tool"
    "patch_fx|patch_fx.lpr|$TOOLS_DIR|patch_fx|tool"
    "patch_live|patch_live.lpr|$TOOLS_DIR|patch_live|tool"
    "patch_midi|patch_midi.lpr|$TOOLS_DIR|patch_midi|tool"

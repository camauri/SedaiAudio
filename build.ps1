...
if ($Help) {
    ...
    Write-Host "    audiotest            Main audio test application"
    Write-Host "    demo_additive        Additive synthesis demo"
    ...
    Write-Host "    .\build.ps1 -LibOnly                  # Build only library"
    Write-Host "    .\build.ps1 -Clean                    # Clean and rebuild all"
    Write-Host "    .\build.ps1 -Target audiotest         # Build only audiotest"
    Write-Host "    .\build.ps1 -Target audiotest -Clean  # Clean and build audiotest"
...
function Build-Target {
    ...
    # Paths - include both src and test directories
    $opts += "-Fu`"$SrcDir`""
    $opts += "-Fu`"$TestDir`""
...
$allTargets = @{
    ...
    'demo_additive' = @{
        Source = 'demo_additive.pas'
        SourceDir = $TestDir
        Output = 'demo_additive.exe'
        IsTest = $true
    }
    'demo_filters_effects' = @{
        Source = 'demo_filters_effects.pas'
        SourceDir = $TestDir
        Output = 'demo_filters_effects.exe'
        IsTest = $true
    }
    'demo_midi_player' = @{
        Source = 'demo_midi_player.pas'
        SourceDir = $TestDir
        Output = 'demo_midi_player.exe'
        IsTest = $true
    }
    'demo_presets' = @{
